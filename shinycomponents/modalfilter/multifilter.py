...
default_filter = dict(key=None, type="==", value=None)
...
def multifilter_server(input, output, session,
    ...
    def out_multifilter_modal():
        return ui.TagList(
            ...
            ui.row(
                ui.input_text("hidden_max_filter_index", "Max filter index", value=0),
                class_="d-none"
            ),
    ...
    def update_max_filter_index():
        ...
        for i, cs in enumerate(filter_components):
            if cs() is not None and type(cs()) == dict and cs().get("value"):
                max_i = i + 1;
...
def filter_server(input, output, session, keys, df, init_filter=default_filter):
    ...
    def update_value_choices():
        ...
        req(input.in_filter_key(), input.in_filter_type(), not df().empty)
        ...
        if input.in_filter_type() == "in":
            choices = df()[selected_key()].drop_duplicates().sort_values().tolist()
            ui.update_select("in_filter_value", choices=choices, selected=current_value)



    @reactive.Effect
    def update_key_choices():
        logger.debug(f"Updating key choices for {current_namespace()}")
        ui.update_select("in_filter_keys", choices=keys())
    ...
    def out_filter_modal():
        ...
        logger.debug(f"Current (key, type, value) : ({current_key}, selected_type(), {current_value})")
        ...
        if selected_type() == "in":
            logger.debug(f"Creating select input for value with initially selected {current_value} ")
            content.append(
                ui.column(
                    3,
                    ui.input_select("in_filter_value", "Filter Value(s)",
            ...
            logger.debug(f"Creating text input for value with initially selected {current_value} ")
            content.append(
                ui.column(
                    3,
